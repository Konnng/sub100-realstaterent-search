...
const webDbFile = path.join(__dirname, '../web/app/data/db.json')
...
const Scrapper = {
  ...
  process () {
    ...
    co(function* () {
      ...
      for (let i = 0; i < Scrapper.vars.realStateTypes.length; i++) {
        ...
        Scrapper.vars.result = Scrapper.vars.result.concat(result || [])
        _log('done processing page => 1 - results found =>', result.length, ' - total =>', Scrapper.vars.result.length)
        ...
        for (let i = 0; i < pages.length; i++) {
          ...
          // process found results in page and filter for non-false objects in the return array
          result = Scrapper.processResponse(response.body) || []

          Scrapper.vars.result = Scrapper.vars.result.concat(result)

          _log('done processing page =>', page + 1, ' - results found =>', result.length, ' - total =>', Scrapper.vars.result.length)
          sleep(100)
        }
    ...
    }).then(result => Scrapper.saveResults())
  },
  *makeRequest (url, callback) {
  ...
  processResponse (body) {
    ...
    // Filter results by the corresponding real estate companies
    ...
    // process returned items, maping the array of results, filtering false items
    return (result || []).map((element) => Scrapper.processResultItem(element)).filter(item => item !== false)
  },
  processResultItem (item) {
    ...
    try {
      ...
      // check the general result array to see if the result already exists
      if (Scrapper.vars.result.filter(item => id === item.id).length > 0) {
        return false
      }
  ...
  processResult () {
    let result = Scrapper.vars.result
  ...
  saveResults () {
    let result = Scrapper.processResult()
...
function _log () {
  ...
  if (time === _logLastTime) {
    timeStr = ' '.repeat(time.length + 2)
  } else {
    timeStr = `[${time}]`
  }
  console.log.apply(console, [].concat([`${timeStr} =>`], Array.from(arguments) || []))
  _logLastTime = time
}
